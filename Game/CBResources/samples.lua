...
local CBE=package.loaded["CBEffects.Library"]
local ParticleHelper=package.loaded["CBEffects.ParticleHelper"]
	...
	samples[1].sampleVGroup=CBE.VentGroup{
		{
			...
			onCreation=function(particle)
				particle.velX=particle.velX+(math.random(-10, 10)/25); particle.velY=particle.velY+(math.random(-10, 10)/25) -- Give a bit of random jitter
			end,
			perEmit=6,
	...
	samples[2].sampleVGroup=CBE.VentGroup{
		{
			...
			onCreation=function(particle)
				particle.velX=particle.velX+(math.random(-10, 10)/25); particle.velY=particle.velY+(math.random(-10, 10)/25)
			end,
			color={{255, 255, 0}},
			positionType="atPoint",
			perEmit=6,
...
samples[3].initiate=function()
	samples[3].sampleVGroup=CBE.VentGroup{
		{preset="flame", physics={relativeToSize=false}}, -- Raw preset loading
		{preset="smoke", physics={relativeToSize=false}}
	}
...
samples[4].initiate=function()
	samples[4].sampleVGroup=CBE.VentGroup{
		{
			preset="default",
	...
	samples[4].sampleVGroup:startMaster()
end
samples[4].stop=function()
	samples[4].sampleVGroup:destroyMaster()
...
samples[5].initiate=function()
	samples[5].sampleVGroup=CBE.VentGroup{
		{
			preset="waterfall",
	...
	samples[5].sampleVGroup:startMaster()
end
samples[5].stop=function()
	samples[5].sampleVGroup:destroyMaster()
...
samples[6].initiate=function()
	samples[6].sampleVGroup=CBE.VentGroup{
		{
			preset="sparks",
			positionType="atPoint",
			build=function()
	...
	samples[6].sampleVGroup:startMaster()
end
samples[6].stop=function()
	samples[6].sampleVGroup:destroyMaster()
...
samples[7].initiate=function()
	samples[7].sampleVGroup=CBE.VentGroup{
		{
			preset="default",
	...
	samples[7].sampleVGroup:startMaster()
end
samples[7].stop=function()
	samples[7].sampleVGroup:destroyMaster()
...
samples[8].initiate=function()
	samples[8].sampleVGroup=CBE.VentGroup{
		{
			preset="hyperspace",
			build=function()
	...
	samples[8].sampleVGroup:startMaster()
end
samples[8].stop=function()
	samples[8].sampleVGroup:destroyMaster()
...
samples[9].initiate=function()
	samples[9].sampleVGroup=CBE.VentGroup{
		{
			preset="wisps",
			x=0,
			y=0,
	...
	samples[9].sampleVGroup:startMaster()
end
samples[9].stop=function()
	samples[9].sampleVGroup:destroyMaster()
...
samples[10].initiate=function()
	samples[10].sampleVGroup=CBE.VentGroup{
		{
			preset="default",
			...
			onUpdate=function(particle)
				if particle._numUpdates>=3 then
					particle.rotateToVel=false -- Turn it off because of unexpected behavior from no velocity; it does some jittery rotation at the very end without this
				end
			end,
			perEmit=12,
			positionType="inRadius",
	...
	samples[10].sampleVGroup:startMaster()
	samples[10].sampleVGroup:emit("default")
end
samples[10].stop=function()
	samples[10].sampleVGroup:destroyMaster()
...
samples[11].initiate=function()
	samples[11].sampleVGroup=CBE.VentGroup{
		{
			preset="default",
			color={{255, 255, 255}},
			build=function()
	...
	samples[11].sampleVGroup:startMaster()
end
samples[11].stop=function()
	samples[11].sampleVGroup:destroyMaster()
