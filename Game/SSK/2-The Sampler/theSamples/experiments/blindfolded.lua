...
function gameLogic:createScene( screenGroup )
	...
	-- Middle Row
	local aBlade = createBlade( centerX + tileSize * 1.4, centerY,  tileSize )
...
--	local aBlade = createBlade( centerX + tileSize, centerY,  tileSize )
	aBlade:scale( 0.6, 0.6 )
...
addInterfaceElements = function()
	...
	ssk.inputs:createVirtualJoystick( centerX, centerY, 25, 5, 15, 
										"myJoystickEvent", backImage, layers.interfaces )
...
createBlade = function ( x, y, size )
	local blade  = ssk.display.imageRect( layers.path, x, y, imagesDir .. "blade4.png",
		                                   { size=size } ) 
	return blade
end
...
onTurnRight = function( event )
	...
	pathRotator:setReferencePoint( display.CenterReferencePoint )
...
onForward = function( event )
	...
	path:setReferencePoint( display.CenterReferencePoint )
